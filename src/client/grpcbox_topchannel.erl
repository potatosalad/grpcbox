...
start_link(Name, Targets, Options) when is_atom(Name) andalso is_list(Targets) andalso is_map(Options) ->
...
% %%%===================================================================
% %%% grpcbox_channel callbacks
% %%%===================================================================

% -spec get_state(pid(), boolean()) -> grpcbox_channel:state().
...
init({Parent, Name, Targets, Options0}) when is_list(Targets) ->
    ...
    0 = map_size(Options1),
    case grpcbox_channelz:register_channel(root, atom_to_binary(Name, unicode), Target) of
        {ok, NodeCtx} ->
            PoolBalancer =
                case Balancer of
                    grpclb -> round_robin;
